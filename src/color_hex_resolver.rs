...
static SUPPLEMENTARY_COLORS: LazyLock<HashMap<&'static str, &'static str>> = LazyLock::new(|| {
    HashMap::from([
...
pub fn resolve(name: &str) -> Option<String> {
    ...
    if let Some(hex) = try_lookup(&normalized) {
        return Some(hex);
    }
    ...
    let words: Vec<&str> = name
        .split([' ', '-', '_', '(', ')', '+', '/'])
        .filter(|w| !w.is_empty())
        .filter(|w| !QUALIFIER_WORDS.iter().any(|q| q.eq_ignore_ascii_case(w)))
        .collect();

    if !words.is_empty() {
        let stripped_normalized = normalize(&words.join(" "));
        if stripped_normalized != normalized
            && let Some(hex) = try_lookup(&stripped_normalized)
        {
            return Some(hex);
        }
    }
    ...
    for word in words.iter().rev() {
        if let Some(hex) = try_lookup(&normalize(word)) {
            return Some(hex);
        }
    }
...
fn try_lookup(normalized: &str) -> Option<String> {
    CSS_COLORS
        .get(normalized)
        .or_else(|| SUPPLEMENTARY_COLORS.get(normalized))
...
mod tests {
    ...
    #[test]
    fn resolve_matches_a_plain_css_color_name() {
        assert_eq!(resolve("Black"), Some("#000000".to_string()));
    }

    #[test]
    fn resolve_matches_case_insensitively() {
        assert_eq!(resolve("HOT PINK"), Some("#FF69B4".to_string()));
    }
    ...
    #[test]
    fn resolve_matches_supplementary_filament_color_words() {
        assert_eq!(resolve("Kraft"), Some("#C09A6B".to_string()));
    }

    #[test]
    fn resolve_returns_none_for_an_unrecognized_name() {
        assert_eq!(resolve("Zorplex Nine"), None);
    }
}
